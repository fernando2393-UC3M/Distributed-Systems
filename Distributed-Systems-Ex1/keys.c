...
int get_value(char *key, char *value1, float *value2)
{
  ...
  if (messagePassing(2, key, value1, *value2) == 0)
  { // Message processing properly done
...
int messagePassing(int operation_code, char *key, char *value1, float value2)
{
  ...
  if (strlen(key) > MAXSIZE) // Check key length to avoid errors when operating
  {
    printf("Key Size too big!   Maximum allowed is %d\n", MAXSIZE);
    return -1;
  }

  if (strlen(value1) > MAXSIZE)  // Check value1 lenght to avoid errors when operating
  {
    printf("Value1 too big!   Maximum allowed is %d\n", MAXSIZE);
    return -1;
  }
